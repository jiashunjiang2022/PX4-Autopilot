...
constexpr ModeConfig source_bound_config(const ModeConfig &config, bool source_identity_match)
{
	...
	if (!source_identity_match) {
		...
		result.constant_r_enabled = false;
...
constexpr ModeConfig sample_config(const ModeConfig &config, bool source_identity_match,
		bool adaptive_quality_usable)
{
	ModeConfig result = source_bound_config(config, source_identity_match);

	if (!adaptive_quality_usable) {
		result.adaptive_r_enabled = false;
	}
