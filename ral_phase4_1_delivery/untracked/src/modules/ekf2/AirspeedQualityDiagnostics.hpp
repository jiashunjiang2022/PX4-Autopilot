...
inline void set_observation_diagnostic(ekf2_airspeed_quality_s &diagnostic, const estimator::airspeedSample &sample,
		uint64_t observation_timestamp_sample, uint64_t quality_timestamp_sample, uint64_t observation_time,
		...
		uint32_t quality_device_id)
{
	...
	diagnostic.quality_age_us = (quality_timestamp_sample > 0 && observation_time >= quality_timestamp_sample)
				    ? static_cast<uint32_t>(math::min(observation_time - quality_timestamp_sample,
